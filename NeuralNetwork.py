# FAST 2-PHASE PIPELINE (RESUMABLE) + Z-SCORE EDGE DETECTOR + SPIKE FILLER
#
# SPIKES:
...
#   every .xlsx write (partial flushes and the final write), so rows restored from
#   older checkpoints that predate the spike columns get filled too.

import os, glob, math, random, json, tempfile, time
...
# ---- Atomic write retry (Windows file-lock robustness) --------------------
# Number of times to retry os.replace() if it fails with PermissionError
# (WinError 5), and the base delay (seconds) between attempts, before giving up.
ATOMIC_WRITE_RETRIES = 6
ATOMIC_WRITE_RETRY_DELAY_S = 0.25
...
def atomic_write_bytes_via(write_fn, final_path, retries=ATOMIC_WRITE_RETRIES, delay=ATOMIC_WRITE_RETRY_DELAY_S):
    ...
    ([WinError 5] Access is denied) if antivirus, OneDrive/sync, search
    indexing, or a lingering handle briefly locks the destination file. This
    almost always clears within milliseconds, so we retry a few times with a
    short backoff before giving up and raising.
    """
...
def main():
    ...
    for xp, jobs in jobs_by_excel.items():
        ...
        for i, j in enumerate(jobs):
            ...
            # This is now NON-FATAL: if the checkpoint write still fails after
            # all retries (e.g. the file is genuinely stuck open somewhere),
            # we warn and keep going rather than aborting the whole run. Worst
            # case that one row's progress isn't saved to disk yet and will be
            # recomputed on the next resume -- cheap, since inference is fast.
            try:
                atomic_write_json(ckpt_path, checkpoint)
            except PermissionError as e:
                print(f"  [warn] could not write checkpoint after row {i} "
                      f"(will retry on next row): {e}")
            ...
            if (i + 1) % XLSX_FLUSH_EVERY_ROWS == 0:
                ...
                try:
                    df_partial = fill_spikes(pd.DataFrame(partial_rows, columns=OUT_COLS))
                    atomic_write_xlsx(df_partial, out_xlsx)
                    print(f"  [flush] {os.path.basename(out_xlsx)}  ({i+1}/{len(jobs)} rows done so far)")
                except PermissionError as e:
                    print(f"  [warn] could not flush partial xlsx at row {i+1} "
                          f"(will retry at next flush point): {e}")

        out_rows = [checkpoint[str(i)] for i in range(len(jobs))]
        df_out = fill_spikes(pd.DataFrame(out_rows, columns=OUT_COLS))
        try:
            atomic_write_xlsx(df_out, out_xlsx)
            print(f"[write] {os.path.basename(out_xlsx)}  ({len(df_out)} rows)  ->  {out_xlsx}")
        except PermissionError as e:
            # Final write failing is worth surfacing loudly since it's the deliverable --
            # but the checkpoint JSON already has everything, so nothing is lost; just
            # close whatever has the file locked and rerun to regenerate the .xlsx.
            print(f"  [ERROR] could not write final {os.path.basename(out_xlsx)}: {e}")
            print(f"  All results are safely stored in {os.path.basename(ckpt_path)} -- "
                  f"close any program that has the xlsx open and rerun to regenerate it.")
